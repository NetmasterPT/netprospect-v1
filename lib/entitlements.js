...
// Path resolvido LAZY (a cada loadPlans) — permite override por PLANS_CONFIG após o import (testes) e
// mantém o comportamento em produção (a env é fixada uma vez).
const plansPath = () => process.env.PLANS_CONFIG ||
  path.resolve(path.dirname(fileURLToPath(import.meta.url)), '../config/plans.json');

let _cfg = null;
export function loadPlans() {
  if (!_cfg) _cfg = JSON.parse(fs.readFileSync(plansPath(), 'utf8'));
  return _cfg;
}
