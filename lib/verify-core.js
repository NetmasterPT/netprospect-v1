...
const DAY = 86400000;
function reverifyAfter(status, cls) {
  ...
  if (status === 'unknown') return isBigProvider(cls) ? null : at(5); // big-provider (só-API) não melhora por re-sondar; corp → cedo
...
export async function verifyDomain(client, { domain, contacts, companyId = null }, { providers, reacher, maxCand = 4, dry = false, mxCache = null, deadlineMs = 0 } = {}) {
  ...
  const persist = async (c, patch) => {
    ...
    const meta = { mail_provider: cls, reverify_after: reverifyAfter(patch.email_status, cls) };
