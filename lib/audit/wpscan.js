...
export function runWpscan(url, { token = process.env.WPSCAN_API_TOKEN, keyless = false, timeoutMs = 240000 } = {}) {
  return new Promise((resolve, reject) => {
    ...
    // Egresso residencial DEDICADO ao wpscan: WPSCAN_PROXY (não o EGRESS_PROXY partilhado) para que só
    // o wpscan saia pelo exit node residencial — o lighthouse (mesmo worker) fica DIRETO, senão as
    // páginas pesadas comiam a bandwidth de casa. WPSCAN_PROXY=http://tailscale-egress:1055 → laptop.
    const proxy = (process.env.WPSCAN_PROXY || process.env.EGRESS_PROXY || '').trim();
