...
export async function decodeHtmlBody(r) {
  ...
  let text;
  try { text = new TextDecoder(cs, { fatal: false }).decode(bytes); }
  catch { text = new TextDecoder('utf-8', { fatal: false }).decode(bytes); }
  // Charset MENTIROSO (header diz utf-8 mas os bytes são Latin-1/Windows-1252 — ex.: sagitta.se): a decode
  // deixa U+FFFD ('�'). UTF-8 VÁLIDO nunca gera U+FFFD → se aparece, os bytes não são o que foi declarado.
  // Re-tenta em windows-1252 e adota-o SÓ se sair limpo (não piora UTF-8 legítimo, que já não teria U+FFFD).
  if (text.includes('�') && cs !== 'windows-1252') {
