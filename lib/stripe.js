...
// Segredo de assinatura do webhook (endpoint secret do Stripe Dashboard) — por modo. Vazio = webhook
// não verificável (recusa-se a processar, fail-closed).
export function stripeWebhookSecret() {
  const isSandbox = (process.env.STRIPE_MODE || '').toLowerCase() !== 'live';
  return isSandbox ? (process.env.STRIPE_TEST_WEBHOOK_SECRET || '') : (process.env.STRIPE_LIVE_WEBHOOK_SECRET || '');
}
