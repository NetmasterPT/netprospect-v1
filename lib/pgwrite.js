...
const SITE_COLS = new Set([
  ...
  'social_facebook', 'social_instagram', 'social_linkedin', 'social_twitter', 'gmb', 'gmb_signal', 'gmb_place_id', 'gmb_name',
  ...
  'mobile_friendly', 'wp_vuln_count', 'security_findings', 'security_severity', 'audit_status', 'audit_error', 'cheap_checked_at',
...
// companies — só os campos gerais que os handlers tocam (general_email/general_phone).
const COMPANY_COLS = new Set(['general_email', 'general_phone', 'name', 'website', 'source']);
export async function pgUpdateCompany(id, patch) {
  ...
  await getPool().query(`UPDATE companies SET ${sets.join(', ')} WHERE id = $1`, [id, ...cols.map((c) => patch[c])]);
}
...
const SITE_COL_TYPE = {
  ...
  gmb_reviews: 'int', seo_score: 'int', mobile_score: 'int', wp_vuln_count: 'int', security_findings: 'int',
