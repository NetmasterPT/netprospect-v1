...
export const CONSUMERS = {
  ...
  // (/api/autoscale) validou 6→12→20→40. Mas a 40 os workers ai/browser do hel1 entraram em rajada de
  // reciclagem (instabilidade Chrome, assinatura do incidente 20260716) e puxaram lighthouse demais para
  // o de1 (6 cores, load ~9) → baixado para 30 (equilíbrio throughput/estabilidade; o fix 3c7b886 trata a
  // perda). A conc por-host (LIGHTHOUSE_CONC) limita a fatia de cada máquina (laptop capado em 3).
  // Override-ável por LIGHTHOUSE_MAX_ACK (default 30): todos os hosts browser fazem ensure deste consumer.
  lighthouse_desktop: { durable: 'lighthouse_desktop', filter: SUBJECTS.lighthouseDesktop, ackWait: 300, maxDeliver: 3, maxAckPending: +(process.env.LIGHTHOUSE_MAX_ACK || 30), role: 'browser' },
  lighthouse_mobile: { durable: 'lighthouse_mobile', filter: SUBJECTS.lighthouseMobile, ackWait: 300, maxDeliver: 3, maxAckPending: +(process.env.LIGHTHOUSE_MAX_ACK || 30), role: 'browser' },
