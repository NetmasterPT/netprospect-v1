...
const SITE_JSON = new Set(['hostnames', 'tech_detected', 'social', 'qualified_reasons', 'lead_score_breakdown', 'ssllabs_report']);
...
const SITE_COLS = new Set([
  ...
  'ip_city', 'contacts_checked_at', 'has_email', 'has_valid_email', 'has_valid_corp_email', 'has_phone', 'business_city', 'business_region', 'business_address', 'social',
  ...
  'traffic_rank', 'traffic_bucket', 'spf_status', 'dmarc_status', 'industry', 'industry_confidence', 'industry_checked_at', 'locality_checked_at', 'seo_score', 'mobile_score',
  'mobile_friendly', 'perf_mobile', 'perf_desktop', 'wp_vuln_count', 'wpscan_checked_at', 'lighthouse_mobile_checked_at', 'lighthouse_desktop_checked_at', 'security_findings', 'security_severity', 'audit_status', 'audit_error', 'cheap_checked_at',
  'audit_checked_at', 'has_decision_maker', 'qualified_reasons', 'lead_score', 'lead_score_breakdown', 'lead_score_at',
  'ssl_issuer', 'ssl_not_after', 'ssl_days_left', 'ssl_grade', 'ssl_validation', 'ssl_wildcard', 'ssllabs_grade', 'ssllabs_checked_at', 'ssllabs_report', 'whois_registrar', 'domain_created', 'domain_expiry',
...
const COMPANY_COLS = new Set(['general_email', 'general_phone', 'phones', 'name', 'website', 'source',
  'reg_number', 'reg_country', 'employees', 'reg_industry', 'reg_checked_at']); // Fase 5 — enriquecimento por registo
...
const CONTACT_COLS = ['name', 'role', 'role_category', 'email', 'phone', 'phone_country', 'social_profiles', 'source', 'source_detail', 'company', 'site', 'gdpr_basis', 'email_off_domain'];
...
// reextract (poison-DB): apaga os contactos-MÁQUINA OBSOLETOS de um site ANTES de reinserir os corrigidos
// (o dedup é insert-only → sem apagar, as linhas envenenadas do extractor antigo ficavam para sempre).
// PRESERVA: revistos à mão (reviewed), DNC/excluídos (do_not_contact), verificados (email_status not null —
// mantém o trabalho do Reacher + TTLs) e os ligados a um email enviado/agendado (emails.contact — não
// orfaniza histórico de campanha). Devolve o nº apagado.
export async function pgDeleteStaleSiteContacts(siteId) {
  const { rowCount } = await getPool().query(
    `DELETE FROM contacts
       WHERE site = $1 AND source = 'site' AND coalesce(reviewed, false) = false
         AND coalesce(do_not_contact, false) = false AND email_status IS NULL
         AND id NOT IN (SELECT contact FROM emails WHERE contact IS NOT NULL)`,
    [siteId]);
  return rowCount;
}
...
const SITE_COL_TYPE = {
  ...
  is_live: 'bool', redirects_www: 'bool', qualified: 'bool', has_email: 'bool', has_valid_email: 'bool', has_valid_corp_email: 'bool', has_phone: 'bool',
  social_facebook: 'bool', social_instagram: 'bool', social_linkedin: 'bool', social_twitter: 'bool', gmb: 'bool',
  is_cpanel: 'bool', mobile_friendly: 'bool', has_decision_maker: 'bool', expiring_soon: 'bool', cms_outdated: 'bool', ssl_wildcard: 'bool',
  checked_at: 'ts', contacts_checked_at: 'ts', cheap_checked_at: 'ts', audit_checked_at: 'ts', lead_score_at: 'ts', gmb_checked_at: 'ts',
  locality_checked_at: 'ts', industry_checked_at: 'ts', wpscan_checked_at: 'ts', lighthouse_mobile_checked_at: 'ts', lighthouse_desktop_checked_at: 'ts',
  ssl_not_after: 'ts', domain_created: 'ts', domain_expiry: 'ts', whois_checked_at: 'ts', ssllabs_checked_at: 'ts',
};
