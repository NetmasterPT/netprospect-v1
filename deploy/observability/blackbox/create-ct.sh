#!/usr/bin/env bash
set -uo pipefail
CTID=204; HOST=de1-blackbox; IP=10.10.10.14
TMPL=local:vztmpl/debian-13-standard_13.1-2_amd64.tar.zst
if ! pct status $CTID >/dev/null 2>&1; then
  echo "[mk] a criar CT $CTID ($HOST)"
  pct create $CTID $TMPL --hostname $HOST --cores 1 --memory 512 --swap 256 \
