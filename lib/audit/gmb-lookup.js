...
export async function lookupGmb({ name, city, domain }, { chromePath = process.env.CHROME_PATH, timeoutMs = 30000, hardTimeoutMs = 60000 } = {}) {
  ...
  try {
    ...
    // DIAGNÓSTICO: devolve a razão do null (URL final + h1) p/ ver onde falha (o handler loga).
    const dbg = (reason) => ({ name: null, _debug: { reason, url: (data?.mapsUrl || '').slice(0, 140), h1: (data?.name || '').slice(0, 60), list: listCount } });
    if (!data || !data.name) return dbg('no-h1');
    // GUARDA anti-lixo: anúncios/consentimento/resultados (h1 = "Porquê este anúncio?", "Definições"…).
    ...
    if (BAD_NAME.test(data.name.trim())) return dbg('bad-name');
    ...
    if (!data.category && !data.rating && !data.address && !data.phone) return dbg('no-signal');
