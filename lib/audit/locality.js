...
// Exportado para o parser GMB (gmb-lookup.js) rejeitar país no campo cidade — mesma regra, uma fonte de verdade.
export const isCountryName = (s) => !!(s && COUNTRY_RE.test(String(s).trim()));

// Localidade ESTRANGEIRA (não-mercado): rodapés de agências web asiáticas metem a SUA morada
// (ex.: ".nl feito por agência VN" → city="Quận 12"/"Thành phố Hồ Chí Minh"). Nenhum mercado do pipeline
// (PT/ES/FR/Nórdicos/NL/DE) usa estes scripts/tokens. PRECISO: só scripts não-europeus + tokens admin VN —
// NUNCA apanha diacríticos europeus (ö/å/ç/ø), apóstrofos ('s-Hertogenbosch) nem mojibake (deixa isso p/ o fetch).
const FOREIGN_SCRIPT_RE = /[Ѐ-ӿ؀-ۿ฀-๿぀-ヿ㐀-鿿가-힯]/; // Cirílico, Árabe, Tailandês, Kana, CJK, Hangul
const VN_TOKEN_RE = /(^|\s)(qu[aậ]n|ph[uư][oờ]ng|đư[oờ]ng|th[àa]nh ph[ốo]|h[àa] n[ộo]i|g[òo] v[ấa]p)(\s|\d|$)/i;
export const isForeignLocality = (s) => !!(s && (FOREIGN_SCRIPT_RE.test(String(s)) || VN_TOKEN_RE.test(String(s))));
const cleanCity = (s) => (s && !isForeignLocality(s) ? notCountry(s) : null);

function titleCity(s) {
  const c = clip(s, 120);
  if (!c) return null;
  // Corta pontuação/espaço à cauda ("Ucklum." → "Ucklum") antes das guardas (país + estrangeiro).
  const city = c.split(/[,;|\n]/)[0].trim().replace(/[.\s]+$/, '').slice(0, 120) || null;
...
function fromPostalAddress(addr) {
  ...
  const city = cleanCity(clip(addr.addressLocality, 120));
