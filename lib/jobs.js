...
export const CONSUMERS = {
  ...
  // PAUSADO 2026-07-19: backlog de ~100k a martelar a PG pública do crt.sh (crt.sh:5432), que está em
  // baixo (ETIMEDOUT) → desperdício. `paused:true` → nenhum worker o ensure/consome. Jobs preservados.
  // Reverter = remover `paused` quando o crt.sh recuperar (ou migrar para a API HTTPS do crt.sh).
  subdomains: { durable: 'subdomains', filter: SUBJECTS.subdomains, ackWait: 120, maxDeliver: 3, maxAckPending: +(process.env.SUBDOMAINS_MAX_ACK || 16), role: 'base', paused: true },
