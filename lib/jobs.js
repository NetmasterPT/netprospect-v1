...
//   browser = Chromium (lighthouse.*, gmb)
...
export const CONSUMERS = {
  ...
  contacts: { durable: 'contacts', filter: SUBJECTS.contacts, ackWait: 90, maxDeliver: 4, role: 'base' },
  audit_ondemand: { durable: 'audit_ondemand', filter: SUBJECTS.auditOndemand, ackWait: 300, maxDeliver: 3, maxAckPending: 8, role: 'browser' },
  audit_qualified: { durable: 'audit_qualified', filter: SUBJECTS.auditQualified, ackWait: 300, maxDeliver: 3, maxAckPending: 8, role: 'browser' },
  audit_rest: { durable: 'audit_rest', filter: SUBJECTS.auditRest, ackWait: 300, maxDeliver: 3, maxAckPending: 8, role: 'browser' },
  ...
  fingerprint: { durable: 'fingerprint', filter: SUBJECTS.fingerprint, ackWait: 30, maxDeliver: 4, role: 'base' },
  ...
  industry: { durable: 'industry', filter: SUBJECTS.industry, ackWait: 180, maxDeliver: 3, maxAckPending: 4, role: 'ai' },
  lighthouse_desktop: { durable: 'lighthouse_desktop', filter: SUBJECTS.lighthouseDesktop, ackWait: 120, maxDeliver: 3, maxAckPending: 2, role: 'browser' },
  lighthouse_mobile: { durable: 'lighthouse_mobile', filter: SUBJECTS.lighthouseMobile, ackWait: 120, maxDeliver: 3, maxAckPending: 2, role: 'browser' },
  gmb: { durable: 'gmb', filter: SUBJECTS.gmb, ackWait: 90, maxDeliver: 2, maxAckPending: 2, role: 'browser' },
  nuclei: { durable: 'nuclei', filter: SUBJECTS.nuclei, ackWait: 200, maxDeliver: 3, maxAckPending: 4, role: 'security' },
  wpscan: { durable: 'wpscan', filter: SUBJECTS.wpscan, ackWait: 300, maxDeliver: 2, maxAckPending: 2, role: 'security' },
};
...
export async function ensureStream(nc) {
  ...
  const cfg = {
    name: STREAM,
    subjects: ['jobs.>'], // cobre coarse + fine-grained (Fase B)
...
export async function ensureConsumer(jsm, spec) {
  const cfg = {
    ...
    backoff: [nanos(5000), nanos(30000), nanos(120000)],
  };
