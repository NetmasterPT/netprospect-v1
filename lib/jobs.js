...
export const SUBJECTS = {
  ...
  registry: 'jobs.registry', // Fase 5 — enriquecimento por registos de empresas (org.nr → decisores + dimensão)
  score: 'jobs.score',
...
export const CONSUMERS = {
  ...
  // REATIVADO 2026-07-20 (keyless, maxAckPending=4) para verificar com TODOS os fixes deployados: handler
  // mark-empty-em-falha (6febf99) + lookup/escrita PG-diretos (a12dea0) + cause-logging do undici (510d679).
  // Histórico: o backfill keyless orfanava com "fetch failed"; investigado a fundo e NÃO reproduz em isolamento
  // (40 .pt sustentado=0 falhas) nem sob carga estável — o burst coincidiu com a tempestade de recreates. Se
  // recorrer, o cause-logging apanha a causa EXATA. Fix definitivo = CERTSPOTTER_API_KEY (TODO-KEYS.md → fonte
  // HTTP rápida, tira o subfinder do caminho crítico). SUBDOMAINS_MAX_ACK override; hosts 'base' fazem ensure.
  subdomains: { durable: 'subdomains', filter: SUBJECTS.subdomains, ackWait: 120, maxDeliver: 3, maxAckPending: +(process.env.SUBDOMAINS_MAX_ACK || 4), role: 'base' },
  ...
  registry: { durable: 'registry', filter: SUBJECTS.registry, ackWait: 45, maxDeliver: 3, role: 'base' }, // Fase 5 — 1 lookup HTTP a registo público
