...
#    Recolha na lib partilhada (metrics-lib.sh): /proc (CPU/RAM/disco/IO/rede) + latências + Docker
#    (containers) + systemd (serviços do sistema base). Falha-suave (só regista).
#    Em LXC (ex.: hel1) o /proc é do host da máquina, não do contentor — igual ao load já reportado.
if [ "$METRICS_ENABLED" = 1 ] && [ -f "$SELF/metrics-lib.sh" ]; then
  # shellcheck source=metrics-lib.sh
  . "$SELF/metrics-lib.sh"
  if collect_and_post >>"$LOG" 2>&1; then log "métricas enviadas"; else log "AVISO envio de métricas falhou"; fi
fi
