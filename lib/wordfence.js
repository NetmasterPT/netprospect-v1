...
function buildIndex(feed) {
  ...
  // plugin/theme SEM protótipo: um slug tipo 'constructor'/'__proto__'/'hasOwnProperty' colidiria com o
  // Object.prototype (truthy não-array) → `||= []` não reatribuía → `.push` rebentava a build inteira.
  const index = { plugin: Object.create(null), theme: Object.create(null), core: [] };
...
export async function matchWpscanVulns(wpscanReport) {
  ...
  const check = (type, slug, version) => {
    ...
    const arr = index[type][String(slug).toLowerCase()]; // guarda: só arrays próprios (após JSON o índice recupera protótipo → slug pode devolver método herdado)
    if (Array.isArray(arr)) for (const v of arr) if (v.ranges.some((rg) => inRange(version, rg))) vulns.push({ type, slug, version, ...v });
  };
