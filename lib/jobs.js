...
export const CONSUMERS = {
  ...
  // maxAckPending = teto FLEET-WIDE de subdomains em paralelo. Subido 4→8 pelo autoscaler (estava PRESO no
  // teto: ~4800 pendentes, ETA ~15h, inflight 4/4; hosts 'base' com folga — laptop 1%/hel1 17%/oracles 45%/23%).
  // NÃO é capped por rate-limit externo: as 8 execuções espalham-se por vários IPs/hosts (crt.sh limita por IP).
  // Override-ável por SUBDOMAINS_MAX_ACK; todos os hosts 'base' fazem ensure (consumers.update a quente).
  subdomains: { durable: 'subdomains', filter: SUBJECTS.subdomains, ackWait: 120, maxDeliver: 3, maxAckPending: +(process.env.SUBDOMAINS_MAX_ACK || 8), role: 'base' },
