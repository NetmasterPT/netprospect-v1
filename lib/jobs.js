...
export const CONSUMERS = {
  ...
  // RE-PAUSADO (2026-07-19). O keyless resiste mesmo a maxAckPending=4 + mark-empty-em-falha + lookup/escrita
  // PG-diretos: o subfinder abre TANTAS conexões que satura os sockets do container → o caminho undici (as
  // fontes HTTP do discoverSubdomains: certspotter/crt.sh) dá "fetch failed" (~480/3min) e orfana. Os fixes
  // do handler (a12dea0: mark-empty + PG read/write) ficam no código para quando reativar COM a fonte que
  // remove a dependência do subfinder: CERTSPOTTER_API_KEY (TODO-KEYS.md) → HTTP rápido, sem hammer de sockets.
