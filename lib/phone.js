...
// Estrito: país SÓ para ccTLDs conhecidos (senão null). Ao contrário de tldToCountry(), NÃO faz
// default a 'PT'. Usado como fallback do geoip: o MaxMind resolve ASN/ISP de IPs anycast/CDN
// (Cloudflare/One.com) mas SEM país → para um .se/.nl/.no/.fi/.pt o país do negócio é o do TLD
// (mais correto que o edge da CDN); um .com atrás de CDN fica null (país desconhecível pelo IP).
export function ccTldCountry(domainOrTld) {
  const tld = String(domainOrTld || '').split('.').pop().toLowerCase();
...
// Rótulos de telefone (multi-língua PT/EN/NL/SE/FI/NO/DE) — um candidato de texto sem
// prefixo internacional só conta como telefone se tiver um destes por perto.
const PHONE_CTX = /(tel|tlf|tlm|phone|mobil|mobiel|mobile|\bmob\b|m[oó]vel|whats|fax|contact|chame|\blig(?:ue|ar|a)\b|puh|ring|call|☎|📞|📱)/i;
...
export function extractPhones(html, { defaultCountry = 'PT', limit = 5 } = {}) {
  ...
  // 2) texto visível — SÓ candidatos com CONTEXTO de telefone perto (tel/telefone/mob/
  // phone/fax/…) e com formatação de telefone. Sem isto, conteúdo numérico (IDs, códigos,
  // datas, valores, tokens de páginas de WAF) valida como "telefone" e envenena o campo.
  if (out.length < limit) {
    const stripped = s.replace(/<script[\s\S]*?<\/script>/gi, ' ').replace(/<style[\s\S]*?<\/style>/gi, ' ');
    TEXT_CAND_RE.lastIndex = 0;
    let c, guard = 0;
    while ((c = TEXT_CAND_RE.exec(stripped)) && out.length < limit && guard++ < 6000) {
      const raw = c[0].trim();
      const digits = raw.replace(/\D/g, '');
      if (digits.length < 8 || digits.length > 15) continue; // fora do intervalo E.164 plausível
      const hasSep = /\d[ ().\-/]+\d/.test(raw); // separador ENTRE dígitos (típico de telefone)
      const intl = /^(?:\+|00)/.test(raw);       // prefixo internacional explícito
      if (!hasSep && !intl) continue;            // dígitos colados sem + → provável ID/valor
      // contexto: uma palavra de telefone nos ~34 chars antes do candidato (cobre "Tel. A, B").
      const before = stripped.slice(Math.max(0, c.index - 34), c.index);
      if (!intl && !PHONE_CTX.test(before)) continue; // sem +intl exige rótulo de telefone perto
      add(tryParse(raw, defaultCountry));
    }
  }
