...
// MAIL FROM por-proxy: cada IP usa o SEU domínio (o do HELO), p/ HELO+MAIL FROM+PTR ficarem alinhados.
// Deriva do domínio registável do HELO (p2.ottomated.online → verify@ottomated.online); override explícito
// via `from` na entrada do proxy; fallback ao REACHER_FROM_EMAIL global se o HELO não tiver domínio.
const FROM_LOCAL = FROM_EMAIL.split('@')[0] || 'verify';
const fromForProxy = (p) => p.from || (getDomain(p.helo || '') ? `${FROM_LOCAL}@${getDomain(p.helo)}` : FROM_EMAIL);
...
export function makeReacherPool({ configPath = PROXIES_PATH, reacherUrl = REACHER_URL } = {}) {
  ...
  async function verify(email, { mxHosts = [], domain } = {}) {
    ...
    stamp(pick, cls, dom);
    const res = await checkEmail(email, { proxy: { host: pick.host, port: pick.port, user: pick.user, pass: pick.pass }, helloName: pick.helo, fromEmail: fromForProxy(pick), reacherUrl });
