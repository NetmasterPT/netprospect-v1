...
export function mapReacher(j) {
  ...
  return { status, reachable, catchAll: !!smtp.is_catch_all, role: !!misc.is_role_account, disposable: !!misc.is_disposable, canConnect: smtp.can_connect_smtp !== false };
}

// Uma verificação. `proxy` = {host,port,user,pass}; `helloName` = PTR do proxy.
export async function checkEmail(email, { proxy, helloName, fromEmail = FROM_EMAIL, reacherUrl = REACHER_URL, timeoutMs = 20000 } = {}) {
