...
export const CONSUMERS = {
  ...
  // Reativado em modo KEYLESS (2026-07-19): subfinder (na imagem) + certspotter-anónimo + crt.sh.
  // As keys (CERTSPOTTER_API_KEY etc., ver TODO-KEYS.md) sobem a cobertura/limite mas não são precisas.
  // maxAckPending BAIXO (4) DE PROPÓSITO: o subfinder abre dezenas de conexões cada; com 12 em paralelo
  // esgotava sockets/fds do worker → o `fetch` do node falhava ("fetch failed") → 99% órfãos. 4 fleet-wide
  // (~1/worker) mantém o node-fetch saudável (escrita Directus + certspotter). ~4×20s = ~17k/dia.
