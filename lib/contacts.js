...
// Honoríficos PT/EN — sinal FORTE de que a linha traz uma pessoa (e não um menu,
// secção ou nome de lugar). Gate da extração nome+cargo em páginas de equipa.
const HONORIFIC = /\b(dr|dra|dr\.?ª|eng|eng\.?ª|arq|arq\.?ª|prof|prof\.?ª|sr|sra|sr\.?ª)\.?\b/i;
...
export function extractPeople(html, sourceUrl, { defaultCountry = 'PT' } = {}) { // eslint-disable-line no-unused-vars
  ...
  if (TEAM_URL.test(sourceUrl || '')) {
    for (let i = 0; i < segments.length && people.length < MAX_PEOPLE_PER_PAGE; i++) {
      ...
      if (!HONORIFIC.test(window)) continue; // exige honorífico → pessoa real, não secção/lugar
      const name = pickName(window.match(NAME_RE));
      if (!name) continue;
