...
export async function createDocument(typeSlug, payload = {}) {
  const cfg = getConfig();
  // VD (Venda a Dinheiro) foi descontinuada pela AT (2013) → qualquer VD é tratada como Fatura-Recibo.
  if (typeSlug === 'venda_dinheiro') typeSlug = 'fatura_recibo';
  const ep = DOC_ENDPOINT[typeSlug];
  if (!ep) throw new Error(`Moloni: tipo sem escrita suportada: ${typeSlug} (${Object.keys(DOC_ENDPOINT).join(', ')})`);
