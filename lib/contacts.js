...
const GENERIC_FIRST = /^(info|geral|general|contact|contactos?|cre|dep|dept|departamento|apoio|suporte|support|reservas|booking|noreply|newsletter|mail|email|webmaster|admin|rh|hr|marketing|comercial|vendas|sales|financeiro|loja|shop|encomendas|ola|hello|servico|servicos|sinistros|clientes|reclamacoes?|qualidade|contabilidade|secretaria|atendimento|factura[cç]ao|cobrancas|direcao|gerencia|tesouraria|ensino|forma[cç]ao|educacao|ci|pessoal|recrutamento|logistica|compras|expedicao|noticias|press|imprensa|gescontact|palaute|hallitus|sihteeri|toimisto|yhdistys|kanslia?|asiakaspalvelu|myynti|tuki|styrelse|styret|kundtj[aä]nst|kundservice|redakti?on|redaksjon|f[oö]rening|hamnvard|kontor|bestuur|secretariaat|klantenservice|kundkontakt|verkstad|tomten)$/i;
...
function deriveName(local) {
  ...
  if (parts.length < 2 || parts.some((p) => GENERIC_FIRST.test(p))) return null; // QUALQUER token departamental → não é pessoa (ex.: ci.admin, sinistros.x, casa.santana.ci)
...
// Remove secções de conteúdo-de-utilizador (comentários/reviews/fórum): os emails/nomes aí são de VISITANTES,
// não da empresa. Sem isto um fórum (ex.: pessoas-que-emprestam-dinheiro.pt) enche o lead com dezenas de gmails
// de terceiros. Best-effort por class/id + markup de comentários WordPress (regex; blocos não-aninhados).
function stripUGC(html) {
  return String(html || '')
    .replace(/<(ol|ul|section|div)\b[^>]*\b(?:id|class)=["'][^"']*(?:comment|coment[aá]rio|reviews?|testimonial|depoiment|f[oó]rum|forum|disqus)[^"']*["'][\s\S]{0,60000}?<\/\1>/gi, ' ')
    .replace(/<article\b[^>]*\bclass=["'][^"']*\bcomment\b[^"']*["'][\s\S]{0,20000}?<\/article>/gi, ' ')
    .replace(/<li\b[^>]*\bid=["']comment-\d+["'][\s\S]{0,20000}?<\/li>/gi, ' ');
}
...
export function extractPeople(html, sourceUrl, { defaultCountry = 'PT' } = {}) { // eslint-disable-line no-unused-vars
  html = stripUGC(html); // A1: fora comentários/reviews/fórum (emails de terceiros, não da empresa)
  ...
  // Nomes derivados dos emails da página (p/ corroborar pessoas de bloco de equipa).
  const emailNames = emails.map((e) => deriveName(e.split('@')[0])).filter(Boolean);
  // Uma pessoa-honorífico só é REAL se corroborada: um email da página deriva o mesmo nome,
  // OU há um LinkedIn que bate com o nome. Senão é uma figura MENCIONADA na prosa (ex.:
  // "Doutor Aníbal Cavaco Silva", "Buzz Aldrin" numa página de história) — não um contacto.
  const corroborated = (name) => !!personSocial(name, liUrls) || emailNames.some((dn) => sameNameish(name, dn) || sameNameish(dn, name));

  // 1) Pares (nome, cargo) em páginas de equipa/sobre — honorífico + corroboração (email/LinkedIn).
  // Sem isso, "N palavras Capitalizadas junto a um cargo" (menus, secções, lugares) ou figuras
  // mencionadas na prosa entravam como contactos. O antigo bloco "nome-só" foi REMOVIDO.
  if (TEAM_URL.test(sourceUrl || '')) {
    for (let i = 0; i < segments.length && people.length < MAX_PEOPLE_PER_PAGE; i++) {
      ...
      if (!corroborated(name)) continue; // sem email/LinkedIn que confirme → figura mencionada, ignora
