...
export async function putSnapshot(siteId, bundle) {
  ...
  // Ponteiro latest: re-tenta 1× (uma falha transitória daqui deixava o getSnapshot a ver
  // null apesar de a versão existir — envenenava o fan-out de sites residenciais/WAF).
  try { await c.putObject(BUCKET, `${siteId}/latest.json`, body, body.length, { 'Content-Type': 'application/json' }); }
  catch { try { await c.putObject(BUCKET, `${siteId}/latest.json`, body, body.length, { 'Content-Type': 'application/json' }); } catch { /* fallback: getSnapshot lê a versão mais recente */ } }
  return key;
}
...
export async function getSnapshot(siteId, key) {
  ...
  try {
    ...
    if (/NoSuchKey|NotFound|does not exist/i.test(e.message || '')) {
      // latest.json em falta (o ponteiro pode ter falhado a escrever) → cai para a versão
      // mais recente. Torna o snapshot robusto à falha do ponteiro (fan-out não fica órfão).
      if (!key) { const v = await listVersions(siteId); if (v.length) return getSnapshot(siteId, v[0]); }
      return null;
    }
    throw e;
  }
