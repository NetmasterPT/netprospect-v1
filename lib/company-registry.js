...
const NO_ORG = /^\d{9}$/;
// Checksum mod-11 do nº de organização NO (pré-filtro barato → não gastar chamadas à API com nºs aleatórios
// de 9 díg, ex. telefones). Pesos 3,2,7,6,5,4,3,2 nos 1.ºs 8 díg; dígito de controlo = 11-(soma%11).
function validNOChecksum(d) {
  const w = [3, 2, 7, 6, 5, 4, 3, 2];
  let sum = 0; for (let i = 0; i < 8; i++) sum += (+d[i]) * w[i];
  const c = 11 - (sum % 11); const chk = c === 11 ? 0 : c;
  return chk !== 10 && chk === +d[8];
}
export function normalizeOrgNumber(raw, country = 'NO') {
  const digits = String(raw || '').replace(/\D/g, '');
  if (country === 'NO') return (NO_ORG.test(digits) && validNOChecksum(digits)) ? digits : null;
  return digits || null;
}

// Extrai o nº de registo do HTML do site (tipicamente no rodapé). ANCORADO a marcadores por país + janela
// limitada (regex-hazard: nunca capturas abertas — ver memória extraction-runon-regex-hazard) + checksum →
// evita apanhar qualquer nº de 9 díg. Devolve { org_number, country } | null. Só NO por agora (é o registo ligado).
export function extractRegNumber(html, { tld } = {}) {
  const t = String(html || '');
  if (!tld || tld === 'no') { // marcador NO + até 20 sep + 9 díg (3-3-3 com espaço/ponto/nbsp opcional)
    const m = t.match(/(?:organisasjonsnummer|foretaksregisteret|org(?:anisasjons)?\.?\s*nr\.?|\bMVA\b|\bNO)[\s:. -]{0,20}(\d{3}[\s. ]?\d{3}[\s. ]?\d{3})/i);
    if (m) { const d = m[1].replace(/\D/g, ''); if (d.length === 9 && validNOChecksum(d)) return { org_number: d, country: 'NO' }; }
  }
  // TODO: SE (Organisationsnummer NNNNNN-NNNN + Luhn), FI (Y-tunnus NNNNNNN-N), NL (KVK 8 díg), PT (NIF 9 díg)
  // — só quando o respetivo registo estiver ligado em lookupByOrgNumber.
  return null;
}
