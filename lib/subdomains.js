...
// certspotter — CT log, grátis (free tier rate-limited). Todas as emissões que cobrem o domínio.
...
export async function discoverSubdomains(domain, { sources = SOURCES } = {}) {
  ...
  const suffix = `.${d}`;
  const results = await Promise.allSettled(sources.map(async ([name, fn]) => ({ name, names: await withTimeout(Promise.resolve(fn(d)), 30000, name) })));
