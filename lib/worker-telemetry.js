...
export async function startTelemetry(extra = {}) {
  ...
  const now = Date.now();
  try {
    await c.hSet(key(''), { id: WID, role: ROLE, host: process.env.HOSTNAME || '', pid: String(process.pid), started: String(now), beat: String(now), ...extra });
    await c.expire(key(''), TTL);
    await c.zAdd('np:wk:index', { score: now, value: WID });
  } catch { /* fail-soft */ }
  const t = setInterval(async () => {
    const cc = await cli(); if (!cc) return;
    try { const n = Date.now(); await cc.hSet(key(''), { beat: String(n) }); await cc.expire(key(''), TTL); await cc.zAdd('np:wk:index', { score: n, value: WID }); } catch { /* */ }
  }, 10000);
  if (t.unref) t.unref();
