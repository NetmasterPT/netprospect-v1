...
// `defaultCountry` (ISO2) resolve telefones nacionais. Agora mantém TODOS os cargos
// e também pessoas SEM cargo detetável (nome-só em páginas de equipa).
export function extractPeople(html, sourceUrl, { defaultCountry = 'PT' } = {}) {
  const text = htmlToText(html);
  const segments = text.split('\n').map((s) => s.trim()).filter(Boolean);
  const people = [];
  const seen = new Set();
  const liUrls = linkedinPersonUrls(html);
  const finish = (p) => { p.role_category = roleCategory(p.role); p.social_profiles = personSocial(p.name, liUrls); return p; };

  // 1a) Pares (nome, cargo) ANCORADOS no cargo — preciso (evita atribuir o cargo
  // da pessoa errada). Só em páginas de equipa/sobre. Cargo na linha i, nome na
  // janela (layout comum "Nome → Cargo": o nome fica logo acima).
  if (TEAM_URL.test(sourceUrl || '')) {
    for (let i = 0; i < segments.length && people.length < MAX_PEOPLE_PER_PAGE; i++) {
      ...
      if (!role) continue;
      const name = pickName([segments[i - 1], segments[i], segments[i + 1]].filter(Boolean).join(' • ').match(NAME_RE));
      if (!name) continue;
    ...
    // 1b) Pessoas SEM cargo detetável (nome-só) que ainda não foram capturadas.
    for (let i = 0; i < segments.length && people.length < MAX_PEOPLE_PER_PAGE; i++) {
      const name = pickName(segments[i].match(NAME_RE));
      if (!name) continue;
      const key = 'n:' + norm(name);
      if (seen.has(key)) continue;
      seen.add(key);
      people.push(finish({ name, role: null, email: null, phone: null, phone_country: null, source_detail: sourceUrl }));
    }
  }

  // 2) Emails — de PESSOA (nome derivável) OU de CARGO; departamentais ignorados.
  for (const email of extractEmails(html)) {
    ...
    if (match) { match.email = email; if (!match.role && role) { match.role = role; match.role_category = roleCategory(role); } }
    else people.push(finish({ name: derived, role, email, phone: null, phone_country: null, source_detail: sourceUrl }));
  }

  // 3) Telefone da página (internacional, E.164) -> 1.º contacto sem telefone.
  const ph = extractPhone(text, { defaultCountry });
  if (ph && people.length) {
    const p = people.find((x) => !x.phone);
    if (p) { p.phone = ph.e164; p.phone_country = ph.country; }
  }
