...
export const CONSUMERS = {
  ...
  subdomains: { durable: 'subdomains', filter: SUBJECTS.subdomains, ackWait: 120, maxDeliver: 3, maxAckPending: +(process.env.SUBDOMAINS_MAX_ACK || 12), role: 'base', paused: true },
  ...
  // PAUSADO 2026-07-19: batch keyless a martelar dezenas de milhar de sites via IP datacenter (risco de
  // reputação de IP). `paused:true` → consumersForRoles exclui-o → nenhum worker o ensure/consome (não
  // recria ao arrancar). Os jobs jobs.wpscan ficam preservados no stream. Reverter = remover `paused`.
  wpscan: { durable: 'wpscan', filter: SUBJECTS.wpscan, ackWait: 300, maxDeliver: 2, maxAckPending: 48, role: 'security', paused: true },
