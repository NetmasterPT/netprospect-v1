...
import { isCountryName, isForeignLocality } from './locality.js';
...
async function buildFromPage(page, domain, via, { strict = false } = {}) {
  ...
  // Moradas formatadas internacionais TERMINAM no PAÍS ("Industrigatan 4, 582 21 Linköping, Suécia"):
  // pegar no último componente às cegas metia o país como cidade (63× "Suécia", 28× "Netherlands", …).
  // Descarta o(s) país(es) à cauda ANTES de escolher a cidade; e nunca deixa 2-partes (rua+cidade) virar região.
  let city2 = null, region = null;
  if (addressRaw) {
    let parts = addressRaw.split(',').map((s) => s.trim()).filter(Boolean);
    while (parts.length > 1 && isCountryName(parts[parts.length - 1])) parts.pop();
    if (parts.length >= 3) { city2 = parts[parts.length - 1]; region = parts[parts.length - 2]; }
    else if (parts.length === 2) { city2 = parts[parts.length - 1]; } // rua + cidade → a 1.ª é rua, não região
    else if (parts.length === 1) { city2 = parts[0]; }
    // Tira o código-postal à cabeça: PT "1000-100", SE "582 21" (dois grupos!), NL "2562 HS", NO/FI/DE "12345".
    if (city2) city2 = city2.replace(/^(\d{3}\s\d{2}|\d{4}\s?[A-Za-z]{2}|\d{3,5}(-\d{2,4})?)\s+/, '').replace(/[.\s]+$/, '').trim();
    if (city2 && (isCountryName(city2) || isForeignLocality(city2))) city2 = null; // país OU rodapé estrangeiro (VN/CJK) → sem cidade
    if (region && (isCountryName(region) || isForeignLocality(region))) region = null;
  }
  return {
    name,
