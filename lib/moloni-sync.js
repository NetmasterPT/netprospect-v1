...
export const SAFT_SLUG = {
  FT: 'fatura', FS: 'fatura_simplificada', FR: 'fatura_recibo', VD: 'fatura_recibo', // VD descontinuada (AT 2013) → tratada como FR
  RE: 'recibo', NC: 'nota_credito', ND: 'nota_debito',
  FF: 'fatura_fornecedor', REF: 'fatura_recibo_fornecedor', VDF: 'fatura_recibo_fornecedor', // VDF → FR fornecedor
  NCF: 'nota_credito_fornecedor', NDF: 'nota_debito_fornecedor', FSF: 'fatura_simplificada_fornecedor',
...
export async function syncDocuments({ dateFrom, documentTypeId } = {}) {
  ...
  for (const d of docs) {
    ...
    const data = {
      ...
      related: Array.isArray(d.associated_documents) ? d.associated_documents.map((a) => a.associated_id).filter(Boolean) : [],
...
// Avenças = documentos SAFT "AV" (cobranças recorrentes por cliente) → moloni_avencas.
export async function syncAvencas() {
  const c = client();
  const types = await moloniCall('/documentTypes/getAll/', {}).catch(() => []);
  const av = (types || []).find((t) => String(t.saft_code || t.name || '').toUpperCase() === 'AV');
  if (!av) return { entity: 'avencas', total: 0, skipped: true, reason: 'sem tipo AV na conta' };
  const docs = await moloniPaginate('/documents/getAll/', { document_type_id: av.document_type_id }, 50, 300);
  const compByMid = await loadMap(c, 'companies', 'moloni_customer_id', docs.map((d) => d.customer_id ?? d.entity?.customer_id));
  const byMid = await loadMap(c, 'moloni_avencas', 'moloni_id', docs.map((d) => d.document_id));
  const ops = [];
  for (const d of docs) {
    const mid = String(d.document_id || ''); if (!mid) continue;
    const custMid = String(d.customer_id ?? d.entity?.customer_id ?? '');
    const comp = custMid ? compByMid.get(custMid) : null;
    const data = {
      name: (d.entity_name || d.entity?.name || 'Avença') + (d.number ? ` #${d.number}` : ''),
      customer_moloni_id: custMid || null,
      amount: d.gross_value != null ? Number(d.gross_value) : null,
      next_date: d.date || null, active: true,
      company: comp ? comp.id : null,
    };
    const match = byMid.get(mid);
    if (match) ops.push(() => c.request(updateItem('moloni_avencas', match.id, data)).then(() => 'updated'));
    else ops.push(() => c.request(createItem('moloni_avencas', { moloni_id: mid, ...data })).then(() => 'created'));
  }
  return { entity: 'avencas', total: docs.length, ...(await batchWrite(ops)) };
}
