...
export const CONSUMERS = {
  ...
  // quase tudo e o Chrome abortava sob contenção (perda ~60%) → baixado p/ 6. Com a frota browser maior
  // (de1 6c + hel1 18c + laptop 22c, todos <25% CPU) e o fix de retry (3c7b886), o recomendador
  // Histórico: 6→12→20→40 (autoscaler). A 40 e a 30 os workers ai/browser do hel1 entraram em rajada de
  // reciclagem (instabilidade Chrome + rajadas Directus under-pressure, assinatura do incidente 20260716)
  // e o fail1h reverteu ~3× em 2 rondas → estabilizado em 20 (o valor onde 20260716 assentou de vez).
  // O fix 3c7b886 trata a perda por retry. Se ainda reciclar a 20, o lever é a conc por-host (LIGHTHOUSE_CONC)
  // no hel1, não o teto. Override-ável por LIGHTHOUSE_MAX_ACK (default 20); todos os hosts browser fazem ensure.
  lighthouse_desktop: { durable: 'lighthouse_desktop', filter: SUBJECTS.lighthouseDesktop, ackWait: 300, maxDeliver: 3, maxAckPending: +(process.env.LIGHTHOUSE_MAX_ACK || 20), role: 'browser' },
  lighthouse_mobile: { durable: 'lighthouse_mobile', filter: SUBJECTS.lighthouseMobile, ackWait: 300, maxDeliver: 3, maxAckPending: +(process.env.LIGHTHOUSE_MAX_ACK || 20), role: 'browser' },
