...
// Timeout DURO por auditoria. Alguns sites penduram o Chrome/CDP indefinidamente (ex.: photobyjb.se >100s
// observado na reabertura do incidente 20260716). Sem isto, o job segura o slot > ackWait(300s) do consumer
// → o NATS re-entrega por timeout → esgota maxDeliver → ÓRFÃO (sem chegar ao ack gracioso). Tem de ficar
// bem abaixo do ackWait p/ o job falhar DENTRO da janela e ser nak'd/desistido honestamente.
const AUDIT_TIMEOUT_MS = Math.max(30000, parseInt(process.env.LIGHTHOUSE_TIMEOUT_MS || '90000', 10));

export async function runLighthouse(url, { chromePath = process.env.CHROME_PATH, formFactor = 'mobile' } = {}) {
  ...
  try {
    ...
    // Desktop: form-factor + emulação de ecrã grande + throttling leve (o default do Lighthouse é
    // MOBILE com 3G+CPU4x). Mobile fica com o default. Corremos os DOIS por site (kinds distintos).
    if (formFactor === 'desktop') {
      opts.formFactor = 'desktop';
      opts.screenEmulation = { mobile: false, width: 1350, height: 940, deviceScaleFactor: 1, disabled: false };
      opts.throttling = { rttMs: 40, throughputKbps: 10 * 1024, cpuSlowdownMultiplier: 1, requestLatencyMs: 0, downloadThroughputKbps: 0, uploadThroughputKbps: 0 };
    }
    // Promise.race com o timeout duro: se a auditoria pendurar, rejeita em AUDIT_TIMEOUT_MS e o `finally`
    // mata o Chrome (o que faz a promise abandonada do lighthouse assentar). O handler apanha o erro e
    // devolve 'retry' → nak/desisto dentro do ackWait, sem órfão. (Reabertura do incidente 20260716.)
    let _lhTimer;
    const _lh = lighthouse(url, opts);
    _lh.catch(() => {}); // se o timeout ganhar a race, o `finally` mata o Chrome e ESTA promise rejeita
    // depois de a race assentar → sem handler seria unhandledRejection (podia derrubar o processo).
    const runnerResult = await Promise.race([
      _lh,
      new Promise((_, reject) => { _lhTimer = setTimeout(() => reject(new Error(`lighthouse timeout ${AUDIT_TIMEOUT_MS}ms`)), AUDIT_TIMEOUT_MS); }),
    ]).finally(() => clearTimeout(_lhTimer));
