...
export const CONSUMERS = {
  ...
  // (/api/autoscale) validou 6→12→20 sem pico de abortos (~7× abaixo do pico do incidente); o
  // recomendador voltou a sugerir dobrar (20→40) com os hosts browser com folga → adotado 40. A conc
  // por-host (LIGHTHOUSE_CONC) limita a fatia de cada máquina (o laptop fica capado em 3, protegido).
  // Override-ável por LIGHTHOUSE_MAX_ACK (default 40): todos os hosts browser fazem ensure deste
  // consumer, por isso o default fleet-wide evita divergência. Vigiar abortos + load do laptop.
  lighthouse_desktop: { durable: 'lighthouse_desktop', filter: SUBJECTS.lighthouseDesktop, ackWait: 300, maxDeliver: 3, maxAckPending: +(process.env.LIGHTHOUSE_MAX_ACK || 40), role: 'browser' },
  lighthouse_mobile: { durable: 'lighthouse_mobile', filter: SUBJECTS.lighthouseMobile, ackWait: 300, maxDeliver: 3, maxAckPending: +(process.env.LIGHTHOUSE_MAX_ACK || 40), role: 'browser' },
