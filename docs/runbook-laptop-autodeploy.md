...
1. Copia `deploy\agent\agent.env.ps1.example` → `deploy\agent\agent.env.ps1` e preenche:
   ...
   ⚠️ **Encoding**: o Windows PowerShell 5.1 lê `.ps1` sem BOM como **ANSI** → acentos/em-dashes
   corrompem-se e podem partir o parse ("The string is missing the terminator") ou o `$REPO` (um path
   com `ç`, ex. `Gonçalo`, fica `GonÃ§alo` = inexistente). O `agent.env.ps1` TEM de estar gravado como
...
```powershell
$ps  = "C:\Users\Gonçalo Pedro\Documents\GitHub\netprospect-v1\deploy\agent\pull-deploy.ps1"
...
- **Guarda docs-only**: se o git pull só trouxer ficheiros `.md` (docs/, raiz), o agente faz o pull
  mas **não recria** (log "só docs .md — sem recreate") — commits de documentação não churnam a frota.
- **`ParserError: missing terminator` / `â€”` no erro (Windows)**: o `.ps1` foi lido como ANSI —
  falta o **BOM UTF-8**. O Windows PowerShell 5.1 só lê UTF-8 corretamente com BOM. Corrige o ficheiro
  com o one-liner da §2.1 (vale para `pull-deploy.ps1` e para o teu `agent.env.ps1`).
